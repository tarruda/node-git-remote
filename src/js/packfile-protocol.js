...
function parseDiscovery(conversation) {
    var discoveryParser, capabilities, head, refCls, inStream = conversation._inStream,
    ...
    discoveryParser = binary().loop(function(end) {
        ...
        this.buffer('length', 4).tap(function(parsed) {
            ...
            this.buffer('sha1', 40)
                ...
                .tap(function(parsed) {
                    var vars, sha1 = parsed.sha1.toString('utf8');
...
function parseFetchData(fetch) {
    var receivingDataParser, k, v, inStream = fetch._inStream,
...
function parseStatus(push) {
    var statusParser, done, inStream = push._inStream,
...
PushDiscoveredRef.prototype.update = function(history) {
    ...
    if (!this._conversation._capabilitiesSent) {
        line = Buffer.concat([
            new Buffer(line), NULL, new Buffer(' ' + pushCapabilities(this._conversation._capabilities))
        ]);
        this._conversation._capabilitiesSent = true;
    } else {
        line += '\n';
